...
- **FR-050**: The branding label font MUST be selected based on the character content of the branding text, with three distinct UI states:
  - **Pure Latin** (no CJK): the settings panel shows a western font picker offering at minimum 6 options — Noto Sans, Noto Serif, Roboto, Oswald, Playfair Display, and Cinzel. All fonts MUST be open-source (SIL OFL or Apache 2.0) and downloaded/cached by `FontAcquisitionService` on first use. Latin-only font files are small (< 300 KB each) and MUST NOT reuse the heavier JP-specific font files.
  - **Pure CJK** (no Latin): the settings panel shows a CJK font picker offering Noto Sans JP and Noto Serif JP.
  - **Mixed** (both CJK and Latin characters present): the settings panel shows BOTH a western font picker and a CJK font picker simultaneously. The selected western font is used for Latin character runs; the selected CJK font is used for CJK character runs. Fallback: any character type with no dedicated font set falls back to the opposing font if it covers that script.
  Character-type detection is performed live on the current branding text value as the user types and the picker UI updates reactively. The selected font values are stored as two separate fields in `OverlaySettings`: `brandingLatinFont` and `brandingCjkFont`.
- **FR-054**: The Rust text renderer MUST support per-character-run font selection for the branding label. Given a branding text string and two optional font handles (Latin font, CJK font), the renderer MUST:
  1. Segment the string into contiguous runs of the same script class (Latin/punctuation vs. CJK ideograph).
  2. Render each run with the appropriate loaded font, measuring each segment's advance width to position the next segment correctly on the same baseline.
  3. Fall back to whichever font is available if only one is loaded.
  4. Fall back to the pixel bitmap font if neither is loaded.
  The `--branding-latin-font` and `--branding-cjk-font` CLI flags carry the resolved file paths for each font slot.
...
- **OverlaySettings**: User-configured rendering preferences. Attributes: branding label enabled (bool), branding label text (string, default "Jellyfin Recents"), video info enabled (bool — master toggle for the entire top-left block), per-line visibility flags for: file size, resolution+fps, video encoding (codec/bit-depth/HDR/colour-space), audio encoding (codec/format/bitrate/track-count), duration; per-frame timestamp badge enabled (bool, default false); timestamp badge position (enum: see FR-036, default "inside-bottom-left"); colour theme (enum: "classic" / "dark" / "light" / "cinematic" / "minimal", default "classic"); font family for overlay info labels and timestamps (enum: "noto-sans-jp" / "noto-serif-jp", default "noto-sans-jp"); **branding Latin font** (enum: "noto-sans" / "noto-serif" / "roboto" / "oswald" / "playfair" / "cinzel", default "noto-sans") — applied to Latin character runs in the branding label; **branding CJK font** (enum: "noto-sans-jp" / "noto-serif-jp", default "noto-sans-jp") — applied to CJK character runs in the branding label. All attributes are included in the overlay cache key hash.
